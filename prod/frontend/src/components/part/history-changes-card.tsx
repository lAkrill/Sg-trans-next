...
import { usePartHistoryActions } from "@/hooks/history-actions.hook";

interface PartHistoryChangesCardProps {
  partId: string;
}
...
export function PartHistoryChangesCard({ partId }: PartHistoryChangesCardProps) {
  const { data: history = [], isLoading, error } = usePartHistoryActions(partId);
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          История изменений технических характеристик детали
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading && (
