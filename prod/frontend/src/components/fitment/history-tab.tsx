...
import { useFitmentHistoryActions } from "@/hooks/history-actions.hook";

interface FitmentHistoryTabProps {
  fitmentId: string;
}
...
export function FitmentHistoryTab({ fitmentId }: FitmentHistoryTabProps) {
  const { data: history = [], isLoading, error } = useFitmentHistoryActions(fitmentId);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            История всех изменений данных арматуры
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading && (
